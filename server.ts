...
app.get("/api/store/data", async (_req, res) => {
  ...
  const isDemo = db.artworks && db.artworks.some((a: any) => a.id && (a.id === 'art-stilo-01' || a.id === 'art-stilo-02'));
  if ((!db.artworks || db.artworks.length === 0 || isDemo) && config.sheetId) {
    try {
      ...
      if (pulled.artworks && pulled.artworks.length > 0) {
        ...
        if (pulled.sales) db.sales = pulled.sales;
        if (pulled.inventoryLogs) db.inventoryLogs = pulled.inventoryLogs;
